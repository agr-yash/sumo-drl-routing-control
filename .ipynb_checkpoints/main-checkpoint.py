...
def get_state(vehicle_id, destination_pos):
    ...
    try:
        ...
        print(
            f"[DEBUG] current_edge_id={current_edge_id}, current_lane_id={current_lane_id}"
        )
    ...
    try:
        ...
        for link in links:
            ...
            print(
                f"[DEBUG] Link to_lane={to_lane}, direction={direction}, to_edge={to_edge}"
            )
            if "r" in direction:
    ...
    for road_key in ["current", "right", "straight", "left"]:
        ...
        if edge_id:
            try:
                ...
                print(
                    f"[DEBUG] {road_key} edge {edge_id} has {len(vehicles_on_edge)} vehicles"
                )
                for v_id in vehicles_on_edge:
                    try:
                        ...
                        print(
                            f"[WARNING] Could not read vehicle {v_id} on edge {edge_id}"
                        )
                        continue
                    if v_type == "AV":
                        av_count += 1
        ...
        print(
            f"[DEBUG] {road_key} stats: AVs={av_count}, HVs={hv_count}, AV_avg={avg_av_speed:.2f}, HV_avg={avg_hv_speed:.2f}"
        )
...
def get_reward(vehicle_id, initial_dist_to_dest, destination_pos):
    ...
    try:
        ...
        print(
            f"[DEBUG] speed={current_speed:.2f}, max_speed={max_speed:.2f}, ds_n={ds_n:.3f}, dd_n={dd_n:.3f}, reward={reward:.3f}"
        )
    ...
    try:
        ...
        for link in links:
            ...
            if direction == chosen_direction:
                ...
                print(
                    f"[INFO] Vehicle {vehicle_id} → {chosen_direction.upper()} to {target_edge}"
                )
                print(f"[DEBUG] Exit execute_action() for {vehicle_id}")
                return
        print(
            f"[WARNING] No link found for desired direction {chosen_direction} from lane {current_lane}"
        )
    except traci.TraCIException as e:
...
def train():
    ...
    print(
        f"[DEBUG] Agent initialized with state_size={STATE_SIZE}, action_size={ACTION_SIZE}"
    )
            ...
            while traci.simulation.getMinExpectedNumber() > 0:
                ...
                for v_id in departed:
                    try:
                        if traci.vehicle.getTypeID(v_id) == "AV":
                            ...
                            if lane_id is None:
                                print(
                                    f"[WARNING] No lane found for dest edge {dest_edge} for vehicle {v_id}"
                                )
                                continue
                            ...
                            print(
                                f"[INFO] AV {v_id} departed: dest_edge={dest_edge}, lane_id={lane_id}, dest_pos={dest_pos}, initial_dist={vehicle_initial_dists[v_id]:.2f}"
                            )
                ...
                for v_id in active_vehicle_ids:
                    try:
                        ...
                        if current_lane.startswith(":"):
                            print(
                                f"[DEBUG] Skipping vehicle {v_id} on internal lane {current_lane}"
                            )
                            continue
                        ...
                        if distance_to_end <= DECISION_ZONE_LENGTH:
                            print(
                                f"[DEBUG] Vehicle {v_id} within decision zone (dist_to_end={distance_to_end:.2f})"
                            )
                            current_state = get_state(v_id, vehicle_dests[v_id])
                            if current_state is None:
                                print(
                                    f"[WARNING] state for {v_id} is None, skipping decision"
                                )
                                continue
                            ...
                            print(
                                f"[DEBUG] Computed reward for {v_id}: {reward:.3f}, cumulative for vehicle: {vehicle_episode_rewards[v_id]:.3f}"
                            )

                            if last_decision_data[v_id] is not None:
                                prev_state, prev_action = last_decision_data[v_id]
                                print(
                                    f"[DEBUG] Agent learning step for {v_id}: prev_action={prev_action}"
                                )
                                agent.step(
                                    prev_state,
                                    prev_action,
                            ...
                            print(
                                f"[INFO] Agent chose action {action} (epsilon={agent.epsilon:.3f}) for vehicle {v_id}"
                            )
                        ...
                        print(
                            f"[WARNING] TraCI error while processing vehicle {v_id}: {e}"
                        )
                        continue
                ...
                for v_id in arrived:
                    if v_id in last_decision_data:
                        ...
                        print(
                            f"[INFO] Vehicle {v_id} arrived: awarding final reward {final_reward:.2f}, cumulative: {vehicle_episode_rewards[v_id]:.2f}"
                        )
                ...
                if min_expected == 0:
                    print(
                        f"[INFO] All vehicles arrived — ending episode early at step {step}"
                    )
                    break
